...
void bt_chk_and_disp(void)
{
	...
	if (mute_state == MUTE)
	{	
		if (isMuteFlash==0)	ui_led_disp_mode(ui_source_select);
		else ui_led_clear();
...
void ui_handle_vol_dec_long_press(void)
{
    ...
    { //??��??????????
        //????????????????
        wd_start(wdtimer_vol_longpress, CLOCKS_PER_SEC / 8, (wdentry_t)ui_handle_vol_dec_long_press, 0);
    }
}
...
void ui_handle_vol_inc_long_press(void)
{
    ...
    { //??��??????????
        //????????????????
        wd_start(wdtimer_vol_longpress, CLOCKS_PER_SEC / 8, (wdentry_t)ui_handle_vol_inc_long_press, 0);
    }
}
