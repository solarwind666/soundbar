...
function build_kernel()
{
    ...
    make "PRODUCT_OUT=$PRODUCT_OUT" clean
    make "PRODUCT_OUT=$PRODUCT_OUT" "SYS_VERSION=$1"
    make_ret=$?
...
function build_bootloader()
{
    ...
    make "$BOOTLOADER_MAKEARGS" "BUILD_CHIP=$BUILD_CHIP" "PRODUCT_OUT=$PRODUCT_OUT"
    make_ret=$?
    popd > /dev/null
...
do
	...
	if [ $moreArgs == "--kernel" ]; then
		cp build/nxap_tools/tools/bootload $PRODUCT_OUT/bootload
		mkdir -p $BUILD_OUT/tools && cp -fr build/nxap_tools/tools $BUILD_OUT/tools/imgmk
	fi
    fi

    BUILD_BOARD_CONFIG="id.boot/bootloader/boot/configs/${BUILD_CHIP}-dvt.cfg"
